...
if (!function_exists('generate_pdf'))
{
    ...
    //    return $data;
        ...
        $report_header = base64_encode(file_get_contents('public/img/report_header.jpg'));
        $report_background = base64_encode(file_get_contents('public/img/report_background.png'));
        $report_footer = base64_encode(file_get_contents('public/img/report_footer.jpg'));
        $report_header = 'data:'.mime_content_type('public/img/report_header.jpg').';base64,'.$report_header;
        $report_background = 'data:'.mime_content_type('public/img/report_background.png').';base64,'.$report_background;
        $report_footer = 'data:'.mime_content_type('public/img/report_footer.jpg').';base64,'.$report_footer;
        ...
        $pdf->save('public/uploads/pdf/'.$pdf_name);//save pdf file

        return url('public/uploads/pdf/'.$pdf_name);//return pdf url
    }
}
...
if (!function_exists('print_barcode'))
{
    function print_barcode($group,$number,$barcode_image)
    {
        ...
        return url('public/uploads/pdf/'.$pdf_name);
    }
}
...
function customer_registration_status($status) {
    $statuses = [
        '0' => 'Active',
        '1' => 'In-Active',
        '2' => 'Suspended',
...
function generate_pdf($view, $data)
{
    $file_name = explode('.', $view);
    $pdf = PDF::loadView($view, $data);
    return $pdf->download(last($file_name) . '-' . time() . '.pdf');
}
