...
class AjaxController extends Controller
{
    ...
    */
    public function get_patient_by_code(Request $request)
    {
        if(isset($request->term))
        {
			...
			$date = new \DateTime($patients->dob);
			$now = new \DateTime();
			$interval = $now->diff($date);
			$patients->ageFull = $interval->y . 'Years ' .$interval->m . 'Months ' .$interval->d .' Days';
        }
        else{
    ...
    */
    public function get_patient_by_name(Request $request)
    {
        if(isset($request->term))
        {
            $patients=Patient::where('fname','like','%'.$request->term.'%')->take(20)->get();
			$date = new \DateTime($patients->dob);
			$now = new \DateTime();
			$interval = $now->diff($date);
			$patients->ageFull = $interval->y . 'Years ' .$interval->m . 'Months ' .$interval->d .' Days';
    ...
    */
    public function get_patient(Request $request)
    {
		...
		$now = new \DateTime();
		$interval = $now->diff($date);
		$patient->ageFull = $interval->y . ' Years ' .$interval->m . ' Months ' .$interval->d .' Days';
